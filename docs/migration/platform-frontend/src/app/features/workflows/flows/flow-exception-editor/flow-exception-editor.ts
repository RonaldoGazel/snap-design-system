...
import { BpmsFlowException, BpmsFlowRule, UnitProfileCombo } from '../../models/bpms.model';
import { RuleType, RuleScope } from '../../models/bpms.enums';
import { TranslateModule, TranslateService } from '@ngx-translate/core';

@Component({
  selector: 'app-flow-exception-editor',
  standalone: true,
  ...
  templateUrl: './flow-exception-editor.html',
  styleUrl: './flow-exception-editor.css',
})
export class FlowExceptionEditorComponent {
  ...
  readonly exceptions = computed(() => this.flowState.rules().filter(r => r.is_exception));
  ...
  readonly ruleTypeOptions = [
    ...
    { label: 'Exceção', value: RuleType.EXCECAO }
  ];
  ...
  readonly form = this.fb.group({
    ...
    type: [RuleType.EXCECAO as string, Validators.required],
  ...
  openForm(exception?: BpmsFlowRule): void {
    if (exception) {
      this.editingId.set(exception.id);
      this.form.patchValue({ name: exception.name, type: exception.type, scope: exception.scope, action: exception.action ?? '', priority: exception.priority, is_active: exception.is_active });
      const src: UnitProfileCombo[] = [];
      if (exception.source_unit_id && exception.source_profile_id) {
        src.push({ unit_id: exception.source_unit_id, profile_id: exception.source_profile_id, unit: this.orgState.unitMap().get(exception.source_unit_id), profile: this.orgState.profiles().find(p => p.id === exception.source_profile_id) });
      }
      const tgt: UnitProfileCombo[] = [];
      if (exception.target_unit_id && exception.target_profile_id) {
        tgt.push({ unit_id: exception.target_unit_id, profile_id: exception.target_profile_id, unit: this.orgState.unitMap().get(exception.target_unit_id), profile: this.orgState.profiles().find(p => p.id === exception.target_profile_id) });
      }
      this.sourceCombo.set(src);
      this.targetCombo.set(tgt);
    } else {
      this.editingId.set(null);
      this.form.reset({ type: RuleType.EXCECAO, scope: RuleScope.PERMISSAO, priority: 0, is_active: true });
  ...
  onSubmit(): void {
    ...
    if (id) {
      ...
      const newException: BpmsFlowException = {
        ...
        action: value.action ?? undefined, priority: value.priority ?? 0, is_active: value.is_active ?? true, is_exception: true,
      ...
      this.flowState.addException(newException);
    }
    this.closeForm();
  }

  removeException(id: string): void { this.flowState.rules.update(list => list.filter(r => r.id !== id)); }
  ...
  isVedacao(scope: string): boolean { return scope === RuleScope.VEDACAO; }
}
